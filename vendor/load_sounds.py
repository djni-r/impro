...
def piano():
    data = []
    rate = wf.read(os.path.dirname(__file__)+"/resources/piano_sounds/39148__jobro__piano-ff-001.wav")[0]

    for i in range(64):
        data.append(wf.read(os.path.dirname(__file__)+"/resources/piano_sounds/{:d}__jobro__piano-ff-0{:02d}.wav".format(39148+i if i < 44 else 39149+i, i+1))[1])
...
def xylo():
    ...
    for name in os.listdir(os.path.dirname(__file__)+"/resources/xylo_sounds"):
        key = name.split('.')[3]
        data[key] = sf.read(os.path.dirname(__file__)+"/resources/xylo_sounds/"+name)[0]
